...
class BillersDropdown extends StatefulWidget {
  const BillersDropdown({
    super.key,
    this.width,
    this.height,
  ...
  final double? width;
  final double? height;
  final List<WBFetchedBillerStruct>? billers; // WBFetchedBillerStruct
  final String? placeholder;
  final String? initialValue;
  final Future Function(
          String label, String value, WBFetchedBillerStruct selectedBiller)?
      onChanged;
  ...
  @override
  State<BillersDropdown> createState() => _BillersDropdownState();
}

class _BillersDropdownState extends State<BillersDropdown>
  ...
  void initState() {
    super.initState();
    final billers = widget.billers;
    if (billers == null || widget.initialValue == null) return;

    final match =
        billers.where((b) => b.billerCode == widget.initialValue).firstOrNull;

    _selectedValue = match?.billerCode;
  }

  String _getLabel(String value) {
    final biller =
        widget.billers?.where((b) => b.billerCode == value).firstOrNull;

    return biller?.name ?? value;
  }
  ...
  void _openPicker() async {
    ...
    final validBillers =
        billers.where((e) => e.billerCode != null && e.name != null).toList();
    ...
    final result = await showModalBottomSheet<String>(
      ...
      builder: (context) => _AlphaGroupedSheet(
        options: validBillers.map((e) => e.billerCode!).toList(),
        labels: validBillers.map((e) => e.name!).toList(),
        title: widget.sheetTitle ?? widget.placeholder ?? 'Select Biller',
      ),
    );
    ...
    final selectedBiller =
        validBillers.where((b) => b.billerCode == result).firstOrNull;

    if (selectedBiller == null) return;

    final label = selectedBiller.name ?? result;

    setState(() => _selectedValue = result);

    // ✅ FIXED: pass 3 params
    await widget.onChanged?.call(label, result, selectedBiller);
  }

  @override
  void didUpdateWidget(covariant BillersDropdown oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (oldWidget.initialValue == widget.initialValue &&
        identical(oldWidget.billers, widget.billers)) {
      return;
    }

    final billers = widget.billers;

    if (billers == null || widget.initialValue == null) {
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: InkWell(
              onTap: (widget.billers?.isNotEmpty ?? false) ? _openPicker : null,
              borderRadius: BorderRadius.circular(8),
              child: Padding(
                ...
                child: Row(
                  children: [
                    Expanded(
                      child: Text(
                        _selectedValue == null
                            ? ((widget.billers?.isEmpty ?? true)
                                ? 'No billers available'
                                : (widget.placeholder ?? 'Select Biller'))
...
class _AlphaGroupedSheetState extends State<_AlphaGroupedSheet> {
  ...
  void _buildStructure() {
    ...
    for (int i = 0; i < widget.labels.length; i++) {
      // final letter = widget.labels[i].isNotEmpty ? widget.labels[i][0].toUpperCase() : '#';

      final label = widget.labels[i].trim();
      final first = label.isEmpty ? '#' : label[0].toUpperCase();
          ...
          children: [
            ...
            Padding(
              ...
              child: SizedBox(
                height: 48,
                child: Semantics(
                  label: 'paybills_biller_search_field',
                  child: TextField(
                    ...
                    child: _filteredRows.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, i) {
                              ...
                              // ── Bank row (no avatar) ──
                              return Semantics(
                                label: 'paybills_biller_${row.text}',
                                button: true,
                                child: InkWell(
                                  onTap: () => Navigator.pop(
                                    context,
                                    widget.options[row.index!],
                                  ),
                                  child: Padding(
